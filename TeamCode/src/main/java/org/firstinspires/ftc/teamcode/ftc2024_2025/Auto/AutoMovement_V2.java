...
import static org.firstinspires.ftc.teamcode.ftc2024_2025.Auto.robotv3_2024_2025Auto.*;
...
AutoMovement_V2 {
    ...
    public AutoMovement_V2(LinearOpMode opMode){
        this.opMode = opMode;
        odo = opMode.hardwareMap.get(GoBildaPinpointDriver.class,"odo");
        odo.setOffsets(0, 3.175); // cneter of pod to center of robot
    ...
    public void forward(double dist){
        ...
        double startpos = odo.getEncoderX();
        double distanceToGo = dist;
        while (Math.abs(distanceToGo) > (this.errorTolerance + 0.5) && opMode.opModeIsActive()) {
            odo.update();

            distanceToGo =  dist - (-odo.getEncoderX() - startpos);
            double power = powerCurving(distanceToGo);
            robotv3_2024_2025Auto.SetPower(power, power, power, power);
            opMode.telemetry.addData("Pos,", distanceToGo);
            opMode.telemetry.addData("encoderpos", odo.getEncoderX());
            opMode.telemetry.addData("power", power);
            opMode.telemetry.update();


        }
        robotv3_2024_2025Auto.SetPower(0,0,0,0);
    }
    ...
    public double convertTicksToInch(double ticks){
        return ((ticks / 2000) * (32*Math.PI)/25.4);
    }
    ...
    public void goToPointConstantHeading(double targetX, double targetY){
        ...
        while((Math.abs(targetXDistance) > this.errorTolerance || Math.abs(targetYDistance) > this.errorTolerance) && opMode.opModeIsActive()){
            ...
            robotv3_2024_2025Auto.SetPower(v1,v3,v2,v4);
